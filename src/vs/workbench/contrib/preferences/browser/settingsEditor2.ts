...
/**
 * 判断一个设置组是否属于第一方 Axon 配置（键以 `axon.` 开头）。
 * 这类设置虽由内置 Axon 扩展贡献，但应作为顶层「Axon」分类展示，而非归入「Extensions」节点。
 */
function isFirstPartyAxonGroup(group: ISettingsGroup): boolean {
	if (!group.extensionInfo) {
		return false;
	}
	const allSettings = group.sections.flatMap(section => section.settings);
	return allSettings.length > 0 && allSettings.every(s => s.key === 'axon' || s.key.startsWith('axon.'));
}
...
export class SettingsEditor2 extends EditorPane {
	...
	private async onConfigUpdate(keys?: ReadonlySet<string>, forceRefresh = false, triggerSearch = false): Promise<void> {
		...
		for (const group of groups) {
			if (group.extensionInfo && !isFirstPartyAxonGroup(group)) {
				extensionSettingsGroups.push(group);
			} else {
				// 第一方 Axon 设置当作核心组处理，从而匹配 tocData 的顶层「Axon」分类，不落入「Extensions」
