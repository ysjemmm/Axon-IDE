...
export class ChatService extends Disposable implements IChatService {
	...
	constructor(
		...
		this._sessionModels = this._register(instantiationService.createInstance(ChatModelStore, {
			...
			willDisposeModel: async (model: ChatModel) => {
				...
				const reqCount = model.getRequests().length;
				const hasDraft = hasDraftInput(model);
				console.warn(`[axon-debug][persist] willDisposeModel session=${model.sessionResource.toString()} localSessionId=${localSessionId ?? '<external>'} reqCount=${reqCount} hasCustomTitle=${!!model.customTitle} hasDraft=${hasDraft} shouldStore=${this.shouldStoreSession(model)} saveModelsEnabled=${this._saveModelsEnabled}`);
				if (localSessionId && this.shouldStoreSession(model)) {
					...
					if (model.getRequests().length === 0 && !model.customTitle) {
						console.warn(`[axon-debug][persist] willDisposeModel -> deleteSession (empty, no title) session=${model.sessionResource.toString()}`);
						await this._chatSessionStore.deleteSession(localSessionId);
					} else if (this._saveModelsEnabled) {
						console.warn(`[axon-debug][persist] willDisposeModel -> storeSessions session=${model.sessionResource.toString()}`);
					...
					console.warn(`[axon-debug][persist] willDisposeModel -> storeSessionsMetadataOnly (external) session=${model.sessionResource.toString()}`);
					await this._chatSessionStore.storeSessionsMetadataOnly([model]);
				} else {
					console.warn(`[axon-debug][persist] willDisposeModel -> no-op session=${model.sessionResource.toString()}`);
				}
			}
		}));
	...
	private async loadRemoteSession(sessionResource: URI, location: ChatAgentLocation, token: CancellationToken, debugOwner?: string): Promise<IChatModelReference | undefined> {
		...
		console.warn(`[axon-debug][load-remote] loadRemoteSession got providedSession session=${sessionResource.toString()} historyLen=${providedSession.history.length} isCompleteObs=${providedSession.isCompleteObs?.get()} hasProgressObs=${!!providedSession.progressObs} hasInterrupt=${!!providedSession.interruptActiveResponseCallback}`);
		...
		if (hasProgressStreaming) {
			...
			const createCancellationListener = (token: CancellationToken) => {
				return token.onCancellationRequested(() => {
					console.warn(`[axon-debug][interrupt] interruptActiveResponseCallback fired session=${model.sessionResource.toString()}\nstack:`, new Error().stack);
					providedSession.interruptActiveResponseCallback?.().then(userConfirmedInterruption => {
						console.warn(`[axon-debug][interrupt] interruptActiveResponseCallback resolved session=${model.sessionResource.toString()} userConfirmedInterruption=${userConfirmedInterruption}`);
	...
	async cancelCurrentRequestForSession(sessionResource: URI, source?: string): Promise<void> {
		...
		console.warn(`[axon-debug][cancel-req] cancelCurrentRequestForSession session=${sessionResource.toString()} source=${source}\nstack:`, new Error().stack);
		const pendingRequest = this._pendingRequests.get(sessionResource);
		if (!pendingRequest) {
			if (source !== 'archive') {
				const model = this._sessionModels.get(sessionResource);
