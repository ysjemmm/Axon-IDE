...
class AgentHostChatSession extends Disposable implements IChatSession {
	...
	override dispose(): void {
		...
		if (!this._store.isDisposed) {
			console.warn(`[axon-debug][session-dispose] AgentHostChatSession.dispose() session=${this.sessionResource.toString()}\nstack:`, new Error().stack);
...
export class AgentHostSessionHandler extends Disposable implements IChatSessionContentProvider {
	...
	async provideChatSessionContent(sessionResource: URI, _token: CancellationToken): Promise<IChatSession> {
		console.warn(`[axon-debug][provide-content] provideChatSessionContent ENTER session=${sessionResource.toString()}`);
		...
		console.warn(`[axon-debug][provide-content] provideChatSessionContent built session=${sessionResource.toString()} isNewSession=${isNewSession} historyLen=${history.length} hasActiveTurn=${initialProgress !== undefined} activeTurnId=${activeTurnId ?? '<none>'}`);
		const session = this._instantiationService.createInstance(
			...
			() => {
				...
				if (!turnId) {
					console.warn(`[axon-debug][interrupt] AgentHost interrupt callback NO active turn session=${sessionKey}`);
					// No active turn (likely a race with completion). Noop-success.
					return true;
				}
				console.warn(`[axon-debug][interrupt] AgentHost interrupt callback dispatching turnCancelled session=${sessionKey} turnId=${turnId}\nstack:`, new Error().stack);
	...
	private _releaseSessionSubscription(sessionUri: string): void {
		...
		if (ref) {
			console.warn(`[axon-debug][sub-release] _releaseSessionSubscription session=${sessionUri}\nstack:`, new Error().stack);
			this._sessionSubscriptions.delete(sessionUri);
			ref.dispose();
		}
	}
