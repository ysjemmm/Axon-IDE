...
// Axon: 以下 Copilot setup/teardown/配额通知/状态栏 入口均未注册，import 一并注释避免未使用告警
...
// Axon: ChatStatusBarEntry 的注册已注释（见下方 registerWorkbenchContribution2 处），import 一并注释避免未使用报错
...
// Axon: 不注册 Copilot setup/teardown/配额通知相关 UI（标题栏 Sign In、Open in Agents、状态栏 "Use AI Features" 等）
...
// Axon: 不注册 Copilot 底部状态栏入口（$(copilot) 图标 + "Sign In" + "Use AI Features" 面板）。
// Axon 的 AI 对话入口由 axon-ide 扩展贡献的状态栏按钮提供（点击打开右侧 Axon 对话栏）。
