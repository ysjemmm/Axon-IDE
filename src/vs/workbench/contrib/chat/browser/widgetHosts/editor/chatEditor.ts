...
export class ChatEditor extends AbstractEditorWithViewState<IChatEditorViewState> {
	...
	protected override setEditorVisible(visible: boolean): void {
		console.warn(`[axon-debug][editor-setEditorVisible] visible=${visible} input=${this.input instanceof ChatEditorInput ? this.input.sessionResource?.toString() : '<not chat>'}`);
	...
	override clearInput(): void {
		console.warn(`[axon-debug][editor-clearInput] ChatEditor.clearInput inputResource=${this.input instanceof ChatEditorInput ? this.input.sessionResource?.toString() : '<not chat>'}\nstack:`, new Error().stack);
	...
	override async setInput(input: ChatEditorInput, options: IChatEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<void> {
		console.warn(`[axon-debug][editor-setInput] ChatEditor.setInput oldInput=${this.input instanceof ChatEditorInput ? this.input.sessionResource?.toString() : '<not chat>'} newInput=${input.sessionResource?.toString()}`);
