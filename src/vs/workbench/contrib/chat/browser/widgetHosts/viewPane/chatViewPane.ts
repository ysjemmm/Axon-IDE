...
export class ChatViewPane extends ViewPane implements IViewWelcomeDelegate {
	...
	private async showModel(token: CancellationToken, modelRef?: IChatModelReference | undefined, startNewSession = true): Promise<IChatModel | undefined> {
		...
		console.warn(`[axon-debug][viewpane-showModel] enter old=${oldModelResource?.toString() ?? '<none>'} newRefSession=${modelRef?.object.sessionResource.toString() ?? '<no ref passed>'} startNewSession=${startNewSession}\nstack:`, new Error().stack);
		this.modelRef.value = undefined;
		console.warn(`[axon-debug][viewpane-showModel] released old modelRef (this.modelRef.value=undefined) old=${oldModelResource?.toString() ?? '<none>'}`);
	...
	async loadSession(sessionResource: URI): Promise<IChatModel | undefined> {
		const t0 = Date.now();
		console.warn(`[axon-debug][viewpane-loadSession] enter session=${sessionResource.toString()}\nstack:`, new Error().stack);
