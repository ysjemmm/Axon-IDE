	...
	public async getOrCreateChatSession(sessionResource: URI, token: CancellationToken): Promise<IChatSession> {
		{
			...
			if (existingSessionData) {
				console.warn(`[axon-debug][get-or-create-session] CACHE HIT (returning stale cached session, NOT re-hydrated from backend) session=${sessionResource.toString()} historyLen=${existingSessionData.session.history.length}`);
		...
		// Check again after async provider resolution
		{
			const existingSessionData = this._sessions.get(sessionResource);
			if (existingSessionData) {
				console.warn(`[axon-debug][get-or-create-session] CACHE HIT (post-resolve, stale) session=${sessionResource.toString()} historyLen=${existingSessionData.session.history.length}`);
				return existingSessionData.session;
			}
		}
