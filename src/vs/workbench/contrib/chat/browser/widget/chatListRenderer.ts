...
export class ChatListItemRenderer extends Disposable implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate> {
	...
	private renderChatTreeItem(element: ChatTreeItem, index: number, templateData: IChatListItemTemplate): void {
		...
		if (isResponseVM(element) && index === this.delegate.getListLength() - 1 && (!element.isComplete || element.renderData)) {
			console.warn(`[axon-debug][renderElement] PROGRESSIVE branch session=${element.sessionResource.toString()} reqId=${element.requestId} isComplete=${element.isComplete} hasRenderData=${!!element.renderData} incremental=${incrementalRendering} responseLen=${element.response.value.length}`);
			...
			if (isResponseVM(element)) {
				console.warn(`[axon-debug][renderElement] BASIC branch session=${element.sessionResource.toString()} reqId=${element.requestId} isComplete=${element.isComplete} hasRenderData=${!!element.renderData} responseLen=${element.response.value.length} isLast=${index === this.delegate.getListLength() - 1}`);
	...
	private doNextProgressiveRender(element: IChatResponseViewModel, index: number, templateData: IChatListItemTemplate, isInRenderElement: boolean): boolean {
		...
		// Stop rendering if the widget has switched to a different session
		// (e.g. user switched chat tabs). The timer will be killed, and when the
		// user switches back the list is rebuilt from the model's current state.
		if (!this.viewModel || !isEqual(element.sessionResource, this.viewModel.sessionResource)) {
			console.warn(`[axon-debug][progressive-guard] SESSION MISMATCH bail element=${element.sessionResource.toString()} viewModel=${this.viewModel?.sessionResource.toString() ?? '<none>'}`);
			return true;
		}
