...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private onDidChangeItems(skipDynamicLayout?: boolean) {
		console.warn(`[axon-debug][onDidChangeItems] visible=${this._visible} hasViewModel=${!!this.viewModel} itemCount=${this.viewModel?.getItems().length ?? 0} willRefresh=${this._visible || !this.viewModel}`);
	...
	setVisible(visible: boolean): void {
		...
		console.warn(`[axon-debug][widget-setVisible] visible=${visible} wasVisible=${wasVisible} session=${this.viewModel?.sessionResource.toString() ?? '<none>'} itemCount=${this.viewModel?.getItems().length ?? 0}`);
		...
		if (visible) {
			if (!wasVisible) {
				this.visibilityTimeoutDisposable.value = disposableTimeout(() => {
					...
					if (this._visible) {
						console.warn(`[axon-debug][widget-setVisible] became visible -> onDidChangeItems(true) session=${this.viewModel?.sessionResource.toString() ?? '<none>'} itemCount=${this.viewModel?.getItems().length ?? 0}`);
	...
	setModel(model: IChatModel | undefined): void {
		console.warn(`[axon-debug][widget-setmodel] ChatWidget.setModel oldSession=${this.viewModel?.sessionResource.toString() ?? '<none>'} newSession=${model?.sessionResource.toString() ?? '<none>'}\nstack:`, new Error().stack);
