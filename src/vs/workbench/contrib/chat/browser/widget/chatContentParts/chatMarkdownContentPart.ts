...
import { allowedMarkdownHtmlAttributes, type IRenderedMarkdown, MarkdownRendererMarkedOptions, type MarkdownRenderOptions } from '../../../../../../base/browser/markdownRenderer.js';
...
import { splitMarkdownBlocks } from './chatIncrementalRendering/markdownBlockSplitter.js';
...
/**
 * A top-level markdown block that has been rendered into its own DOM node.
 * Tracked so stable blocks can be reused across re-renders during streaming
 * instead of tearing down and rebuilding the entire markdown DOM.
 */
interface IRenderedMarkdownBlock {
	/** Raw markdown source of this block, including its trailing separator. */
	readonly raw: string;
	/** Root DOM node of the rendered block; a direct child of `domNode`. */
	readonly node: HTMLElement;
	/** Disposables owned by this block (render result, scrollables, refs). */
	readonly store: DisposableStore;
	/** The entries this block contributed to `allRefs`, in order. */
	readonly refs: IDisposableReference<CodeBlockPart | ChatOutputCodeBlockPart | CollapsedCodeBlock | MarkdownDiffBlockPart>[];
	/** The entries this block contributed to `_codeblocks`, in order. */
...
export class ChatMarkdownContentPart extends Disposable implements IChatContentPart {
	...
	/**
	 * Top-level markdown blocks currently rendered, in document order. Enables
	 * block-level DOM reuse: during streaming, blocks whose source text is
	 * unchanged keep their DOM node (and any internal state such as a table's
	 * scroll position) while only changed/new blocks are re-rendered.
	 */
	private readonly _renderedBlocks: IRenderedMarkdownBlock[] = [];
	...
	constructor(
		...
		// A single ResizeObserver bound to this part's lifetime (not per-render),
		// so KaTeX/table layout participants survive block reuse. Each block
		// removes its own participant callbacks when it is torn down.
		const layoutParticipants = new Lazy(() => {
			const observer = this._register(new dom.DisposableResizeObserver('ChatMarkdownContentPart.mathLayout', () => this.mathLayoutParticipants.forEach(layout => layout())));
			this._register(observer.observe(this.domNode));
			return this.mathLayoutParticipants;
		});

		const markdownDecorationsRenderer = instantiationService.createInstance(ChatMarkdownDecorationsRenderer);

		// The store of the block currently being rendered. codeBlockRendererSync
		// registers per-block disposables here so a block can be disposed and
		// re-rendered independently of its stable siblings.
		let currentBlockStore: DisposableStore;
		...
		const doRenderMarkdown = () => {
			...
			// Re-evaluated each render so a deferred KaTeX load is picked up.
			...
			// Renders one top-level block; appends refs/codeblocks to the
			// instance-level arrays, which the caller slices per block afterwards.
			const codeBlockRendererSync = (languageId: string, text: string, raw?: string): HTMLElement => {
				...
				if (languageId === 'diff' && raw && this.rendererOptions.allowInlineDiffs) {
					...
					if (match && isResponseVM(context.element)) {
						...
						currentBlockStore.add(ref);
						return diffPart.element;
					}
				}
				if (languageId === 'vscode-extensions') {
					const chatExtensions = currentBlockStore.add(instantiationService.createInstance(ChatExtensionsContentPart, { kind: 'extensions', extensions: text.split(',') }));
			...
			// Per-block render options; reused for each block rendered below.
			const renderBlock = (blockContent: IMarkdownString): IRenderedMarkdown => {
				return currentBlockStore.add(renderer.render(blockContent, {
					sanitizerConfig,
			...
			// Split into top-level blocks and reuse the unchanged prefix. Because
			// streaming markdown is append-only, every block before the first
			// changed one keeps its DOM node — and thus any internal state such as
			// a table's horizontal scroll position — untouched. The parentElement
			// check guards against the morpher having cleared the DOM out from
			// under us (e.g. the word buffer reseeds from an empty container).
			const blocks = splitMarkdownBlocks(this.markdown.content.value);
			let reuseCount = 0;
			while (
				reuseCount < blocks.length &&
				reuseCount < this._renderedBlocks.length &&
				this._renderedBlocks[reuseCount].raw === blocks[reuseCount] &&
				this._renderedBlocks[reuseCount].node.parentElement === this.domNode
			) {
				reuseCount++;
			}

			// Tear down the changed blocks, salvaging reusable output code block
			// refs so the rebuilt blocks can re-adopt them.
			const reusableOutputCodeBlockRefs = new Map<string, IDisposableReference<ChatOutputCodeBlockPart>>();
			for (let i = reuseCount; i < this._renderedBlocks.length; i++) {
				const staleBlock = this._renderedBlocks[i];
				for (const ref of staleBlock.refs) {
					if (ref.object instanceof ChatOutputCodeBlockPart) {
						staleBlock.store.deleteAndLeak(ref);
						reusableOutputCodeBlockRefs.set(ref.object.reuseKey, ref as IDisposableReference<ChatOutputCodeBlockPart>);
					}
				}
				staleBlock.store.dispose();
				staleBlock.node.remove();
			}
			this._renderedBlocks.length = reuseCount;

			// Re-aggregate allRefs/_codeblocks from the surviving prefix blocks,
			// then resume the global code block index where the prefix left off.
			this.allRefs.length = 0;
			this._codeblocks.length = 0;
			for (const block of this._renderedBlocks) {
				this.allRefs.push(...block.refs);
				this._codeblocks.push(...block.codeblocks);
			}
			globalCodeBlockIndexStart = codeBlockStartIndex + this._codeblocks.length;

			// Render and append the new/changed blocks in document order.
			for (let i = reuseCount; i < blocks.length; i++) {
				const refStart = this.allRefs.length;
				const codeblockStart = this._codeblocks.length;
				currentBlockStore = new DisposableStore();

				const blockContent = new MarkdownString(blocks[i], this.markdown.content);
				blockContent.baseUri = URI.revive(this.markdown.content.baseUri);
				blockContent.uris = this.markdown.content.uris;

				const rendered = renderBlock(blockContent);
				currentBlockStore.add(markdownDecorationsRenderer.walkTreeAndAnnotateReferenceLinks(this.markdown, rendered.element));
				this.makeKatexBlocksScrollable(rendered.element, layoutParticipants, currentBlockStore);
				currentBlockStore.add(wrapTablesWithScrollable(rendered.element, layoutParticipants));

				this.domNode.appendChild(rendered.element);
				this._renderedBlocks.push({
					raw: blocks[i],
					node: rendered.element,
					store: currentBlockStore,
					refs: this.allRefs.slice(refStart),
					codeblocks: this._codeblocks.slice(codeblockStart),
				});
			}
		...
		if (enableMath && !MarkedKatexSupport.getExtension(dom.getWindow(context.container))) {
			...
			MarkedKatexSupport.loadExtension(dom.getWindow(context.container))
				.then(() => {
					// Force a full rebuild: the markdown source is unchanged, so
					// block reuse would otherwise skip re-rendering the blocks with
					// the now-loaded math extension.
	...
	override dispose(): void {
		...
		this._renderedBlocks.length = 0;
		this.allRefs.length = 0;
		this._codeblocks.length = 0;
	}

	/**
	 * Disposes all rendered blocks and clears the DOM, forcing the next
	 * doRenderMarkdown() to rebuild every block from scratch. Used when a
	 * re-render must happen even though the markdown source is unchanged
	 * (e.g. after the KaTeX extension finishes loading).
	 */
	private resetRenderedBlocks(): void {
		for (const block of this._renderedBlocks) {
			block.store.dispose();
		}
		this._renderedBlocks.length = 0;
	...
	private makeKatexBlocksScrollable(root: HTMLElement, layoutParticipants: Lazy<Set<() => void>>, store: DisposableStore): void {
		// eslint-disable-next-line no-restricted-syntax
		for (const katexBlock of root.querySelectorAll('.katex-display')) {
			...
			layoutParticipants.value.add(scan);
			store.add(toDisposable(() => layoutParticipants.value.delete(scan)));
