...
import { AXON_LOGO_PATH } from './axonLogoPath.js';

/**
 * Axon logo "fish" used by the Agents window aquarium. Each fish is a small
 ...
 * a single `<symbol>` containing the Axon logo path. All fish reference
...
function ensureSharedDefs(targetDocument: Document): void {
	...
	container.appendChild(createAxonLogoSymbol(targetDocument));
...
function createAxonLogoSymbol(targetDocument: Document): SVGSymbolElement {
	...
	logoPath.setAttribute('d', AXON_LOGO_PATH);
 ...
 *   - Axon logo body, sliced into N vertical strips that each oscillate in
...
function buildFishSvg(targetDocument: Document): SVGSVGElement {
	...
	// viewBox 0..96 matches the Axon logo glyph coordinate space.
